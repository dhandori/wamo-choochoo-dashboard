...
UA = "Mozilla/5.0 (WAMO-Market-Radar/21.0)"
...
def _fetch_fnguide_business_summary(code):
    """Fetch the actual FnGuide Snapshot Business Summary.
    The simplified URL can return a 'page not found' response; use the same
    parameterized Snapshot URLs that expose Business Summary in the browser.
    """
    code = str(code).zfill(6)
    urls = [
        (
        ...
        (
            "https://comp.fnguide.com/svo2/fncompany.asp?"
            + urllib.parse.urlencode({
                "cId": "CB",
                "gicode": "A" + code,
            })
        ...
        try:
            ...
            for enc in ("utf-8", "euc-kr", "cp949"):
                try:
                    ...
                    if "Business Summary" in candidate or "기업개요" in candidate:
                        html = candidate
                        break
                    if html is None:
                        html = candidate
            ...
            # The browser page has a stable "Business Summary" heading.
            if "Business Summary" not in page_text:
                errors.append("Business Summary heading missing")
                continue

            after = page_text.split("Business Summary", 1)[1]
            ...
            # Exclude date/title/navigation; retain company-specific business sentences.
            candidates = []
            for x in lines:
                if len(x) < 45:
                    continue
                if re.fullmatch(r"\[\d{4}/\d{2}/\d{2}\]", x):
                    continue
                if any(k in x for k in ("시장 다변화", "실적 개선", "성장 지속", "집중")) and len(x) < 60:
                    continue
                if any(k in x for k in ("동사는", "회사는", "사업", "ODM", "OEM", "제조", "판매", "영위")):
                    candidates.append(x)
                if len(candidates) >= 3:
                    break

            # HTML list items are a secondary extraction path.
            if not candidates:
                lis = [
                ...
                candidates = [
                    x for x in lis
                    if len(x) >= 45
                    and any(k in x for k in ("동사는", "회사는", "사업", "ODM", "OEM", "영위"))
                    and not any(k in x for k in ("로그인", "회원가입", "이용약관"))
                ][:3]

            if candidates:
                return candidates, url

            errors.append("summary sentences missing")

        except Exception as e:
            errors.append(str(e))

    raise RuntimeError("FnGuide Business Summary 확보 실패: " + " | ".join(errors[:3]))
...
def profile_enrich(raw):
    ...
    def one(x):
        ...
        try:
            ...
            return x, prof, "FnGuide Business Summary 기반 구조화", None
        except Exception:
            pass
    ...
    return {
        ...
        "source": "FnGuide Business Summary + OpenDART fallback",
