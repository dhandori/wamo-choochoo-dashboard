...
UA = "Mozilla/5.0 (WAMO-Market-Radar/13.0)"
...
def enrich_one_dart(x, corp_code, old=None):
    ...
    if result.get("latest_annual_eps_yoy") is None or result.get("annual_eps_cagr_3y") is None:
        proxy_series, proxy_metric = annual_earnings_fallback_series(corp_code, annual_latest)
...
def main():
    ...
    payload = {
        "meta": {
            ...
            "note": "가격·거래량·시가총액 + OpenDART 공식 EPS·매출·ROE·발행주식수·최근 공시가 연결된 버전입니다. v5에서 금융사 등 EPS 미제공 종목은 연간 이익 대체지표를 명시하고, 주식수 비교 누락 시 최근 1년 희석성 공시를 대체근거로 보강했습니다. 외국인·기관·연기금·사모 5·20·60거래일 수급까지 연결했습니다. 컨센서스 리비전은 아직 미연결입니다.",
        },
        "sectors": sectors,
        "stocks": raw,
        "errors": errors[:100],
