...
def _nasdaq_financial_health(ticker,sector=''):
    """SEC가 GitHub Actions IP를 차단할 때 사용하는 Nasdaq 연간 재무 안전망."""
    symbol=str(ticker or '').replace('-','.').upper()
    url=f'https://api.nasdaq.com/api/company/{urllib.parse.quote(symbol)}/financials?frequency=1'
    payload=us._get_json(url,headers={'Referer':f'https://www.nasdaq.com/market-activity/stocks/{symbol.lower()}/financials'})
    data=(payload or {}).get('data') or {}; income=data.get('incomeStatementTable') or {}; balance=data.get('balanceSheetTable') or {}
    period=((balance.get('headers') or {}).get('value2') or (income.get('headers') or {}).get('value2') or '')
    def row_value(table,label):
        for row in table.get('rows') or []:
            if str(row.get('value1') or '').strip().lower()==label.lower(): return num(row.get('value2'))
        return None
    assets=row_value(balance,'Total Assets'); liabilities=row_value(balance,'Total Liabilities'); equity=row_value(balance,'Total Equity')
    current_assets=row_value(balance,'Total Current Assets'); current_liabilities=row_value(balance,'Total Current Liabilities')
    net_income=row_value(income,'Net Income')
    if liabilities is None and assets is not None and equity is not None: liabilities=assets-equity
    ...
    sector_text=str(sector or '').lower(); financial=any(k in sector_text for k in ('finance','financial','bank','insurance','금융','은행','보험'))
    equity_ok=bool(equity is not None and equity>0); profit_ok=bool(net_income is not None and net_income>0)
    ...
    return {'status':'PASS' if passed else 'FAIL','source':'Nasdaq 연간 재무제표','period':period,
            'sectorRule':'FINANCIAL' if financial else 'GENERAL','equityPositive':equity_ok,'profitPositive':profit_ok,
            'leveragePass':leverage_ok,'liquidityPass':liquidity_ok,'assets':assets,'liabilities':liabilities,
            'equity':equity,'netIncome':net_income,'debtRatioPct':round(debt_ratio,1) if debt_ratio is not None else None,
            'currentRatioPct':round(current_ratio,1) if current_ratio is not None else None,
            'criterion':'SEC 우선 · SEC 접속 불가 시 Nasdaq 연간 재무로 자본·순이익 및 비금융 부채비율을 확인'}
...
def _us_health(ticker,stock,mapping):
    ...
    sec_error='SEC 티커 연결 없음'
    if item:
        try: return us.sec_financial_health_for_mapping(item,stock.get('sector') or stock.get('krxSector') or '')
        except Exception as exc: sec_error=us.core.compact_provider_error(str(exc))
    try: return _nasdaq_financial_health(ticker,stock.get('sector') or stock.get('krxSector') or '')
    except Exception as exc: return {'status':'UNAVAILABLE','source':'SEC EDGAR + Nasdaq','criterion':f'{sec_error} / Nasdaq: {us.core.compact_provider_error(str(exc))}'}
...
def us_top30(um):
    ...
    candidates.sort(reverse=True,key=lambda x:x[0])
    sec_cache=us._load_cache()
    mapping=((sec_cache.get('__company_map__') or {}).get('rows') or {}) if isinstance(sec_cache,dict) else {}
    out=[]; failed=0; unavailable=0
    for pct,ticker,old in candidates:
        ...
        out.append({'ticker':ticker,'displayTicker':old.get('displayTicker') or ticker,'name':name,
                    ...
                    **t,'sector':sector,'industry':industry,'description':desc,'financialHealth':health,
                    'source':'미국 중대형주 정밀계산 유니버스 + '+str(health.get('source') or '재무자료'),'priceProvider':old.get('priceProvider') or old.get('dataSource')})
