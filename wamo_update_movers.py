...
<script>const D={data};let M='korea';const fmt=(x,d=1)=>x==null?'—':Number(x).toLocaleString(undefined,{{maximumFractionDigits:d}});function setMarket(m){{M=m;document.getElementById('krBtn').classList.toggle('on',m==='korea');document.getElementById('usBtn').classList.toggle('on',m==='us');render()}}function render(){{const a=D[M]||[];const md=(D.meta||{{}})[M]||{{}};document.getElementById('meta').textContent=`기준일 ${{md.asOf||'—'}} · 갱신 ${{md.updatedAt||'—'}} · ${{a.length}}종목`;const aligned=a.filter(x=>x.alignmentStatus==='YES').length,near=a.filter(x=>(x.high52Ratio||0)>=90).length,vol=a.filter(x=>(x.volumeRatio20||0)>=1.5).length;document.getElementById('summary').innerHTML=`<div class=kpi><span>상위 종목</span><b>${{a.length}}</b></div><div class=kpi><span>정배열</span><b>${{aligned}}</b></div><div class=kpi><span>52주 고점 90%+</span><b>${{near}}</b></div><div class=kpi><span>거래량 20일평균 1.5배+</span><b>${{vol}}</b></div>`;document.getElementById('grid').innerHTML=a.map((x,i)=>`<article class=card><div class=top><div><div class=rank>#${{i+1}}</div><div class=name>${{esc(x.name)}}</div><div class=ticker>${{esc(x.ticker)}} · ${{esc(x.market)}}</div></div><div class=gain>+${{fmt(x.changePct,2)}}%</div></div><div class=badges><span class="badge ${{x.alignmentStatus==='YES'?'y':x.alignmentStatus==='PENDING'?'p':'n'}}">${{x.alignmentStatus==='YES'?'정배열':x.alignmentStatus==='PENDING'?'정배열 판정보류':'정배열 아님'}}</span><span class=badge>52주 고점 ${{fmt(x.high52Ratio)}}%</span><span class=badge>거래량 ${{fmt(x.volumeRatio20,2)}}x</span></div><div class=sector>${{esc(x.sector||'—')}}</div><div class=desc>${{esc(x.description||'')}}</div><canvas id="c${{i}}" width="430" height="142"></canvas><div class=metrics><div><span>종가</span><b>${{fmt(x.close,2)}}</b></div><div><span>20일선</span><b>${{fmt(x.ma20,2)}}</b></div><div><span>60일선</span><b>${{fmt(x.ma60,2)}}</b></div><div><span>200일선</span><b>${{fmt(x.ma200,2)}}</b></div></div></article>`).join('');requestAnimationFrame(()=>a.forEach((x,i)=>draw(document.getElementById('c'+i),x.history||[])))}}function esc(s){{return String(s??'').replace(/[&<>\"]/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}}[c]))}}function draw(c,h){{if(!c||h.length<2)return;const ctx=c.getContext('2d'),w=c.width,hg=c.height,p=8,vals=h.map(x=>Number(x.close));const sma=n=>vals.map((_,i)=>i+1<n?null:vals.slice(i+1-n,i+1).reduce((a,b)=>a+b,0)/n);const series=[{{v:vals,c:'#69adff',lw:2}},{{v:sma(20),c:'#45d6a0',lw:1.2}},{{v:sma(60),c:'#f1cb6c',lw:1.2}},{{v:sma(120),c:'#d58cff',lw:1.1}},{{v:sma(200),c:'#ff8c7a',lw:1.1}}];const all=series.flatMap(s=>s.v.filter(v=>v!=null));const mn=Math.min(...all),mx=Math.max(...all),span=mx-mn||1;ctx.clearRect(0,0,w,hg);ctx.strokeStyle='#21344e';ctx.lineWidth=1;for(let j=1;j<4;j++){{let y=p+(hg-2*p)*j/4;ctx.beginPath();ctx.moveTo(p,y);ctx.lineTo(w-p,y);ctx.stroke()}}for(const s of series){{ctx.strokeStyle=s.c;ctx.lineWidth=s.lw;ctx.beginPath();let started=false;s.v.forEach((v,j)=>{{if(v==null)return;let x=p+(w-2*p)*j/(vals.length-1),y=hg-p-(v-mn)/span*(hg-2*p);if(!started){{ctx.moveTo(x,y);started=true}}else ctx.lineTo(x,y)}});if(started)ctx.stroke()}}ctx.font='9px system-ui';ctx.fillStyle='#91a4bf';ctx.fillText('종가 · MA20 · MA60 · MA120 · MA200',10,12)}}setMarket('korea');</script></body></html>'''


def patch_nav(path):
    """Keep both dashboard pages linked to movers and show the fixed KST schedule.

    This runs after each market updater, so the generated index/us HTML cannot
    silently lose the TOP30 menu or schedule label on the next automatic refresh.
    """
    p=Path(path)
    s=p.read_text(encoding='utf-8')

    # Remove stale/duplicate schedule labels from earlier patches, then insert one
    # canonical line immediately below the market switch nav.
    s=re.sub(r'\s*<div[^>]+id=["\']wamo-update-schedule["\'][^>]*>.*?</div>\s*', '\n', s, flags=re.S|re.I)
    schedule=(
        '<div id="wamo-update-schedule" '
        'style="margin:-10px 0 14px;color:#91a4bf;font-size:11px;line-height:1.5">'
        '<b style="color:#c9d8ea">자동갱신(KST)</b> · '
        '한국 12:00 / 16:00 · 미국 00:00 / 05:00 · 시장별 하루 2회'
        '</div>'
    )
    nav_end=s.find('</nav>')
    if nav_end >= 0:
    ...
    # Ensure exactly one TOP30 menu item in the market-switch nav.
    s=re.sub(r'\s*<a[^>]+href=["\']movers\.html["\'][^>]*>.*?</a>', '', s, flags=re.S|re.I)
    nav_start=s.find('<nav class="market-switch"')
    nav_end=s.find('</nav>', nav_start if nav_start >= 0 else 0)
    if nav_start >= 0 and nav_end >= 0:
        block=s[nav_start:nav_end]
        us=list(re.finditer(r'<a[^>]+href=["\']us\.html["\'][^>]*>.*?</a>', block, flags=re.S|re.I))
        if us:
            pos=nav_start+us[-1].end()
            s=s[:pos]+'\n    <a href="movers.html">🚀 상승률 TOP 30</a>'+s[pos:]
...
def main():
    ...
    # Patch navigation/schedule first. Even if an external ranking endpoint is
    # temporarily unavailable, the freshly generated dashboards keep the menu.
    patch_nav(ROOT/'index.html'); patch_nav(ROOT/'us.html')
    # First deployment must never expose a 404. Create a valid empty movers page
    # before remote data collection, then replace it with live data on success.
    if not OUT.exists():
    ...
    OUT.write_text(html(cache),encoding='utf-8')
    # Idempotent second pass is deliberate in case page markup changed upstream.
    patch_nav(ROOT/'index.html'); patch_nav(ROOT/'us.html')
    print('Done:',OUT)
