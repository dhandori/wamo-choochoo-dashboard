...
UA = "Mozilla/5.0 (WAMO-Market-Radar/22.0)"
...
def market_direction():
    try:
        ...
        return {
            ...
            "ma50": ma50,
            "ma200": ma200,
            "note": "주가·거래량·6조건·Stage 2·RS·OpenDART 재무/공시 + 섹터 대장주/주요기업 + 네이버 증권 기업개요(FnGuide)와 사업보고서를 바탕으로 핵심사업·제품·고객·수익구조를 투자용으로 구조화해 자동갱신합니다. 수급·컨센서스는 사용자 설정에 따라 사용하지 않습니다.",
        }
    except Exception as e:
...
def profile_enrich(raw):
    ...
    def one(x):
        ...
        # Primary: NAVER Finance company overview (FnGuide-sourced).
        # This route is on the same NAVER domain already used successfully for market data.
        try:
            bullets, naver_industry, _ = _fetch_naver_company_overview(code)
            profile_sector = naver_industry or x.get("sector")
            prof = _profile_from_business_summary(x.get("name"), profile_sector, bullets)
            prof["naverIndustry"] = naver_industry
            return x, prof, "네이버 증권 기업개요(FnGuide) 기반 구조화", None
        except Exception:
            pass
    ...
    return {
        ...
        "source": "NAVER Finance 기업개요(FnGuide) + OpenDART fallback",
