...
UA = "Mozilla/5.0 (WAMO-Market-Radar/10.0)"
...
def annual_eps_series(corp_code, latest_year, old=None):
    # Reuse prior LIVE payload's audited annual series when it is still current.
    old_series = (old or {}).get("annualEpsSeries")
    if isinstance(old_series, list) and old_series:
        years = [int(x.get("year",0)) for x in old_series if x.get("year")]
        if years and max(years) >= latest_year:
            return old_series

    vals = []
    for y in range(latest_year, latest_year - 4, -1):
        rows, fs = dart_statement(corp_code, y, "11011")
        eps = extract_annual_eps(rows) if rows else None
        if eps is not None:
            vals.append({"year": y, "eps": eps, "fs": fs})
    vals.sort(key=lambda z: z["year"])
    return vals
...
def issued_shares(corp_code, year, reprt_code):
    ...
    for r in d.get("list") or []:
        ...
        if v is not None:
            vals.append(v)
    # Rows may be common/preferred classes; total them.
...
def enrich_one_dart(x, corp_code, old=None):
    ...
    # Supply dilution: same report vs prior-year same report.
    if latest_year and latest_reprt:
        cur_sh = issued_shares(corp_code, latest_year, latest_reprt)
        prev_sh = issued_shares(corp_code, latest_year - 1, latest_reprt)
        if cur_sh is not None:
            result["issued_shares"] = cur_sh
        if cur_sh is not None and prev_sh not in (None, 0):
            result["share_growth_yoy"] = (cur_sh / prev_sh - 1) * 100
...
def main():
    ...
    payload = {
        "meta": {
            ...
            "note": "가격·거래량·시가총액 + OpenDART 공식 EPS·매출·ROE·발행주식수·최근 공시가 연결된 버전입니다. 투자자별 수급과 컨센서스 리비전은 다음 단계입니다.",
        },
        "sectors": sectors,
        "stocks": raw,
        "errors": errors[:100],
