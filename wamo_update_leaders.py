...
UA = "Mozilla/5.0 (WAMO-Market-Radar/18.0)"
...
def market_direction():
    try:
        ...
        return {
            ...
            "ma50": ma50,
            "ma200": ma200,
            "note": "주가·거래량·6조건·Stage 2·RS·OpenDART 재무/공시 + 섹터 대장주/주요기업 + 종목별 사업보고서 비즈니스모델 요약을 자동갱신합니다. 수급·컨센서스는 사용자 설정에 따라 사용하지 않습니다.",
        }
    except Exception as e:
...
def _sector_easy_model(sector):
    s = str(sector or "")
    rules = [
        (["보험"], "보험료를 받고 위험을 인수한 뒤 보험손익과 운용수익으로 돈을 버는 업종입니다."),
        (["은행","금융"], "고객 자금을 조달해 대출·투자하고 예대마진과 수수료에서 수익을 내는 금융 업종입니다."),
        (["증권"], "주식·채권 중개, 자산관리, 투자은행 업무의 수수료와 운용수익이 핵심입니다."),
        (["반도체"], "반도체 또는 관련 부품·장비를 개발·생산해 고객사에 공급하면서 매출을 올리는 업종입니다."),
        (["화장품"], "화장품을 직접 판매하거나 브랜드사를 대신해 개발·생산(ODM/OEM)하여 돈을 버는 업종입니다."),
        (["의약","제약","바이오"], "의약품·바이오 제품의 개발·생산·판매 또는 기술이전으로 수익을 내는 업종입니다."),
        (["자동차"], "완성차 또는 자동차 부품을 생산해 완성차 업체·유통망에 공급하는 업종입니다."),
        (["건설"], "건축·토목 프로젝트를 수주해 공사를 수행하고 공사대금과 개발이익을 얻는 업종입니다."),
        (["조선"], "선박·해양설비를 수주해 건조하고 계약대금을 받는 수주산업입니다."),
        (["기계","장비","전기"], "산업용 기계·장비·전기제품을 제조해 기업 고객에게 판매·공급하는 업종입니다."),
        (["소프트웨어","IT","정보"], "소프트웨어·IT 서비스를 제공하고 라이선스·구독·구축비 등으로 수익을 내는 업종입니다."),
        (["통신"], "통신망을 운영해 가입자의 통신요금과 기업용 네트워크 서비스에서 매출을 올리는 업종입니다."),
        (["유통","소매","도매"], "상품을 매입하거나 중개해 소비자·기업에 판매하고 유통마진을 얻는 업종입니다."),
        (["화학"], "화학 소재·원료를 생산해 다른 제조업체에 공급하면서 매출을 올리는 업종입니다."),
        (["식품","음료"], "식품·음료를 생산하거나 유통해 소비자와 유통채널에 판매하는 업종입니다."),
        (["운송","해운","항공"], "사람이나 화물을 운송하고 운임을 받는 사업이 핵심입니다."),
        (["에너지","전력","가스"], "에너지·전력·가스를 생산·공급하거나 관련 설비를 운영해 수익을 내는 업종입니다."),
    ]
    for keys, desc in rules:
        if any(k in s for k in keys):
            return desc
    return f"{s or '해당'} 업종의 제품·서비스를 고객에게 공급해 매출을 올리는 구조입니다."
...
def profile_enrich(raw):
    ...
    target = [
        x for x in candidates
        if x.get("conditionCount",0) >= 4 or x.get("trendTemplate") or x.get("signal") in ("BUY","HOLD","WATCH")
    ][:PROFILE_TARGET_MAX]
    ...
    for x in raw:
        x["businessModelEasy"] = _sector_easy_model(x.get("sector"))
    ...
    for x in target:
        ...
        try:
            ...
            fresh = (today - d).days <= 180 and bool(old.get("businessModel"))
        except Exception:
            fresh = False

        if fresh:
            x["businessModel"] = old.get("businessModel")
            x["businessModelSource"] = old.get("businessModelSource","OpenDART 사업보고서")
    ...
    def one(x, rcept):
        txt = _dart_document_text(rcept)
        sec = _business_section(txt)
        sentences = _extract_business_sentences(sec)
        if not sentences:
            raise RuntimeError("사업의 내용 핵심문장 추출 실패")
        return x, " ".join(sentences)

    with ThreadPoolExecutor(max_workers=PROFILE_WORKERS) as ex:
        futs = [ex.submit(one, x, rcept) for x, rcept in jobs]
        for i, fut in enumerate(as_completed(futs), 1):
            try:
                x, summary = fut.result()
                x["businessModel"] = summary
                x["businessModelSource"] = "OpenDART 사업보고서"
                x["businessModelReportDate"] = x.get("businessReportDate")
                cache[x.get("ticker")] = {
                    "businessModel": summary,
                    "businessModelSource": "OpenDART 사업보고서",
                    "businessModelReportDate": x.get("businessReportDate"),
    ...
    # Apply cache to remaining target names if available.
    for x in target:
        if x.get("businessModel"):
            continue
        old = cache.get(x.get("ticker")) or {}
        if old.get("businessModel"):
            x["businessModel"] = old["businessModel"]
            x["businessModelSource"] = old.get("businessModelSource","OpenDART 사업보고서")
            x["businessModelReportDate"] = old.get("businessModelReportDate")

    _save_profile_cache(cache)
    total_has = sum(bool(x.get("businessModel")) for x in target)
    return {
        "status": "LIVE" if total_has >= max(10, int(len(target)*0.60)) else "PARTIAL",
        "targetCount": len(target),
        "coveredCount": total_has,
        "fetchedCount": fetched,
        "source": "OpenDART 사업보고서 원문",
        "message": f"기업 비즈니스모델 {total_has}/{len(target)}개 후보 연결",
        "errors": errors[:15],
    }
...
.business-box{margin:12px 0;background:linear-gradient(135deg,#102945,#0d1c31);border:1px solid #31577e;border-radius:13px;padding:13px}
.business-box .biz-k{font-size:10px;color:#81baff;font-weight:900;letter-spacing:.02em}
.business-box .biz-easy{font-size:13px;font-weight:850;line-height:1.65;margin-top:6px;color:#eff5ff}
.business-box .biz-detail{font-size:11px;line-height:1.7;margin-top:8px;color:#b8c7dc}
.business-box .biz-src{font-size:9px;color:#778ba7;margin-top:8px}
"""
    if ".business-box{" not in html:
        html = html.replace("</style>", css + "\n</style>", 1)

    # 2) Sector card: current technical leader + major companies.
    ...
    # 3) Business-model box in the stock detail drawer.
    box = """<div class="business-box">
      <div class="biz-k">이 회사는 뭘로 돈 버나</div>
      <div class="biz-easy" id="businessEasy">—</div>
      <div class="biz-detail" id="businessModel">—</div>
      <div class="biz-src" id="businessSource">—</div>
    </div>"""
    why = """<div class="whybox"><div class="k">왜 이 판정이 나왔나</div><div id="detailWhy">—</div></div>"""
    if 'id="businessModel"' not in html and why in html:
        html = html.replace(why, why + "\n    " + box, 1)

    # 4) Fill business-model box when clicking a stock.
    js_anchor = """$('#detailWhy').textContent=x.signalReason||'—'; updateDetailWatch();"""
    js_new = """$('#detailWhy').textContent=x.signalReason||'—';
    const be=document.getElementById('businessEasy'), bm=document.getElementById('businessModel'), bs=document.getElementById('businessSource');
    if(be) be.textContent=x.businessModelEasy||`${x.sector} 업종 기업입니다.`;
    if(bm) bm.textContent=x.businessModel||'회사별 사업보고서 핵심 설명은 아직 준비되지 않았습니다.';
    if(bs) bs.textContent=x.businessModel?`근거: ${x.businessModelSource||'OpenDART 사업보고서'}${x.businessModelReportDate?' · '+x.businessModelReportDate:''}`:'업종 기준 한줄 설명만 표시 중';
    updateDetailWatch();"""
    if js_anchor in html and "const be=document.getElementById('businessEasy')" not in html:
        html = html.replace(js_anchor, js_new, 1)

    # 5) User opted out of flow/consensus: remove warning pills if previously injected.
    html = re.sub(
