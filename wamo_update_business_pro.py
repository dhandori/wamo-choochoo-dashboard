...
UA = "Mozilla/5.0 (WAMO-Market-Radar/19.0)"
...
def market_direction():
    try:
        ...
        return {
            ...
            "ma50": ma50,
            "ma200": ma200,
            "note": "주가·거래량·6조건·Stage 2·RS·OpenDART 재무/공시 + 섹터 대장주/주요기업 + 종목별 사업보고서에서 핵심사업·제품·고객·수익구조를 구조화해 자동갱신합니다. 수급·컨센서스는 사용자 설정에 따라 사용하지 않습니다.",
        }
    except Exception as e:
...
def profile_enrich(raw):
    ...
    # Every stock gets a useful structured fallback immediately.
    for x in raw:
        tpl = _sector_profile_template(x.get("sector"))
        x["businessProfile"] = dict(tpl)
    ...
    for x in target:
        ...
        try:
            ...
            fresh = (today - d).days <= 180 and isinstance(old.get("businessProfile"), dict)
        except Exception:
            fresh = False
        ...
        if fresh:
            ...
            x["businessModel"] = old["businessProfile"].get("summary","")
    ...
    def one(x, corp):
        ...
        if not annual or not annual.get("rcept_no"):
            raise RuntimeError(f"{x.get('name')}: 사업보고서 A001 검색 실패")
        txt = _dart_document_text(annual["rcept_no"])
        sec = _business_section(txt)
        if len(sec) < 300:
            raise RuntimeError(f"{x.get('name')}: 사업의 내용 원문 부족")
        prof = _make_business_profile(sec, x.get("sector"))
        return x, annual, prof

    with ThreadPoolExecutor(max_workers=PROFILE_WORKERS) as ex:
        futs = [ex.submit(one, x, corp) for x, corp in jobs]
        for i, fut in enumerate(as_completed(futs), 1):
            try:
                x, annual, prof = fut.result()
                x["businessProfile"] = prof
                x["businessModelEasy"] = prof.get("summary") or x.get("businessModelEasy")
                x["businessModel"] = prof.get("summary","")
                x["businessModelSource"] = "OpenDART 사업보고서 원문"
                x["businessModelReportDate"] = annual.get("date")
                cache[x.get("ticker")] = {
                    "businessProfile": prof,
                    "businessModelSource": "OpenDART 사업보고서 원문",
                    "businessModelReportDate": annual.get("date"),
    ...
    source_count = sum(
        bool((x.get("businessModelSource") or "").startswith("OpenDART"))
        for x in target
    )
    return {
        "status": "LIVE" if source_count >= max(10, int(len(target)*0.60)) else "PARTIAL",
        "targetCount": len(target),
        "coveredCount": source_count,
        "fetchedCount": fetched,
        "source": "OpenDART A001 사업보고서 원문",
        "message": f"회사별 상세 비즈니스모델 {source_count}/{len(target)}개 후보 연결",
        "errors": errors[:20],
    }
...
.business-grid{display:grid;grid-template-columns:1fr;gap:7px;margin-top:10px}
.biz-row{background:#0a1728;border:1px solid #213956;border-radius:9px;padding:9px 10px}
.biz-row .lbl{font-size:9px;font-weight:900;color:#78b5ef;margin-bottom:3px}
.biz-row .val{font-size:11px;line-height:1.6;color:#c7d4e6}
...
"""
    ...
    box = """<div class="business-box">
      ...
      <div class="business-grid">
        <div class="biz-row"><div class="lbl">주요 제품·서비스</div><div class="val" id="businessProducts">—</div></div>
        <div class="biz-row"><div class="lbl">고객·판매처</div><div class="val" id="businessCustomers">—</div></div>
        <div class="biz-row"><div class="lbl">돈 버는 구조</div><div class="val" id="businessRevenue">—</div></div>
        <div class="biz-row"><div class="lbl">실적이 좋아지는 조건</div><div class="val" id="businessDrivers">—</div></div>
      </div>
    ...
    const be=document.getElementById('businessEasy'), bp=document.getElementById('businessProducts'), bc=document.getElementById('businessCustomers'), br=document.getElementById('businessRevenue'), bd=document.getElementById('businessDrivers'), bs=document.getElementById('businessSource');
    const p=x.businessProfile||{};
    if(be) be.textContent=p.summary||x.businessModelEasy||`${x.sector} 업종 기업입니다.`;
    if(bp) bp.textContent=p.products||'핵심 제품·서비스 정보 확인 필요';
    if(bc) bc.textContent=p.customers||'주요 고객·판매처 정보 확인 필요';
    if(br) br.textContent=p.revenue||'매출 구조 정보 확인 필요';
    if(bd) bd.textContent=p.drivers||'핵심 실적 변수 확인 필요';
    if(bs) bs.textContent=x.businessModelSource?`근거: ${x.businessModelSource}${x.businessModelReportDate?' · '+x.businessModelReportDate:''}`:'업종 템플릿 기반 설명';
    updateDetailWatch();"""
