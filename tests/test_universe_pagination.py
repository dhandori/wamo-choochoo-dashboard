...
class PaginationTests(unittest.TestCase):
    ...
    def test_third_sweep_can_reconcile_rank_movement(self):
        responses = iter(([page(range(100)), page([99], 2)] * 2) +
                         [page(list(range(99)) + [100]), page([99], 2)])
        rows = fetch_naver_universe(lambda *a, **k: next(responses), 1, '.KQ', 'KOSDAQ')
        self.assertEqual(len(rows), 101)

    def test_persistent_gap_fails_after_four_complete_sweeps(self):
        responses = iter([page(range(100)), page([99], 2)] * 4)
